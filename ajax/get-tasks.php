...
$stmt = $db->prepare("SELECT * FROM `{$table}` WHERE cat_id = {$cat_id} ORDER BY task_start DESC");
$stmt->execute();
...
while ($task = $stmt->fetch(PDO::FETCH_ASSOC) ) {
	if ( $task["task_status"] == 0) {
		...
		$outputToDo .= '<label class="me-1"><span class="icon-size-1 me-1"><svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"></path><circle cx="12" cy="7" r="4"></circle><path d="M6 21v-2a4 4 0 0 1 4 -4h4a4 4 0 0 1 4 4v2"></path></svg></span> <span>'. get_user_name( $task["assigned_to"] ) .'</span></label>';
		$outputToDo .= '</div>';
		$outputToDo .= '</div>';
		$outputToDo .= '</div>';
		$outputToDo .= '</div>';
	...
	elseif ( $task["task_status"] == 1) {
		...
		$outputDone .= '<div>';
		$outputDone .= '<div class="font-weight-medium">'. $task["task_title"] .'</div>';
		$outputDone .= '<div class="text-muted">';
		$outputDone .= '<label class="me-1"><span class="icon-size-1 me-1"><svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-calendar-minus" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"></path><rect x="4" y="5" width="16" height="16" rx="2"></rect><line x1="16" y1="3" x2="16" y2="7"></line><line x1="8" y1="3" x2="8" y2="7"></line><line x1="4" y1="11" x2="20" y2="11"></line><line x1="10" y1="16" x2="14" y2="16"></line></svg></span> <span>'. date("F j, Y, g:i a", strtotime( $task['task_due'] )) .'</span></label> ';
